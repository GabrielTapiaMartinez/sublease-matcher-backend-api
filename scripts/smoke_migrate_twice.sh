...
if [[ -z "${SM_DATABASE_URL:-}" ]]; then
  echo "SM_DATABASE_URL must be set to run the migration smoke test." >&2
  exit 1
fi

echo "[smoke-sql-twice] Resetting database..."
bash scripts/db_reset.sh

echo "[smoke-sql-twice] Re-running alembic upgrade head (should be no-op)..."
PYTHONPATH=src python -m alembic upgrade head

echo "[smoke-sql-twice] Re-seeding demo data (idempotent)..."
PYTHONPATH=src python scripts/seed_db.py

echo "[smoke-sql-twice] Starting API server..."
SERVER_LOG="$(mktemp -t smoke-sql-twice-XXXX.log)"
...
echo "[smoke-sql-twice] Waiting for server..."
for _ in {1..30}; do
...
echo "[smoke-sql-twice] All checks passed."
